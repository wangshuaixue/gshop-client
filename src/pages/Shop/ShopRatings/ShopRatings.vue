...
<script>
  ...
  export default {  //配置对象
    data () {
      return {
        selectType: 2, // 2全部 , 1不满意, 0满意
        onlyContent: true, // 是否只显示有内容的
      }
    },
    mounted () {
    ...
    computed:{
      ...
      filterRatings () {
        const {ratings, selectType, onlyContent} = this
          ...
          /*if(selectType===2 ){  //全部显示
            if(!onlyContent){
              return ratings.filter((rating)=>rating.text)
            }else{
              return ratings.filter((rating)=>rating.text.length>0)
            }
          }else if(selectType===0 ){  //满意 : 分情况看有没有被选中
            if(!onlyContent){
              return ratings.filter((rating)=>rating.rateType===0 && rating.text)
            }else{
              return ratings.filter((rating)=>rating.rateType===0 && rating.text.length>0)
            }

          }else{  //不满意
            if(!onlyContent){
              return ratings.filter((rating)=>rating.rateType===1 && rating.text)
            }else{
              return ratings.filter((rating)=>rating.rateType===1 && rating.text.length>0)
            }
          }*/
          // 条件1
          /*
